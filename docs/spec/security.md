...
### `commitgate check` never writes to git

The `commitgate check` command (and its hook invocations) reads the staging area and executes checks. It never calls `git add`, `git commit`, `git push`, `git reset`, or any other write operation. A bug in `commitgate check` cannot corrupt git history.

### `commitgate mcp` has a controlled, limited write surface

The `commitgate mcp` server calls `git add <files>` before running the pipeline (to populate the staging area for built-in checks), and either `git commit -m <message>` on success or `git restore --staged <files>` on failure. These are the only git write operations it performs. The write surface is:

- `git add <files>`: stages the files provided by the agent
- `git restore --staged <files>`: unstages those same files on failure or gate error
- `git commit -m <message>`: commits only when all checks pass

The MCP server never calls `git push`, `git reset --hard`, or any operation that modifies existing commits or the remote. A bug that causes the commit to execute when checks have not passed is explicitly guarded against (see CG-MCP-04 assertion). The staging area is always restored to its pre-call state on any non-success outcome.
